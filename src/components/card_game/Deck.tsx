...
function init(tiles: TileAssetT[]): t {
  return {
    cards: (shuffleArray(tiles.concat(tiles)) as unknown as TileAssetT[])
      .map((tile: TileAssetT, index: number) => Card().init(tile, index)),
    selectedCards: []
  }
}
...
function hideAllCardsExcept(deck: t, card: CardT): t {
  return {...deck, cards: [...deck.cards.filter((_card: CardT) => _card.id !== card.id).map((card: CardT) => hideCard(card)), card]}
}

function guessCardEffect(deck: t, card: CardT): t {
  return {
    ...deck, ...hideAllCardsExcept(deck, card), afterEffect: null
  }
}

function missCardEffect(deck: t): t {
  return {
    ...deck, ...hideAllCards(deck), afterEffect: null
  }
}

function revealSelectedCard(cards: CardT[], card: CardT): CardT[] {
  return cards.map((_card: CardT) => _card.id === card.id ? revealCard(_card) : _card)
}

async function processCard(e: BaseSyntheticEvent, card: CardT, deck: t, cardApi: CardApiType): Promise<t> {
  ...
  switch (selectedCards.length) {
    ...
    case 1:
      ...
      if (_selectedCards.length === 1) {
        ...
        _deck = {..._deck, selectedCards: [], afterEffect: guessCardEffect(_deck, card)}
      }
      // Did not guessed the card
      if (_selectedCards.length === 2) {
        console.log("Did not guessed the card: ", card)
        // cardApi.showCard(e.target.parentNode)
        _deck = {..._deck, selectedCards: [], afterEffect: missCardEffect(_deck)}
      }
      break
